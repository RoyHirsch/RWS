...
class MetricLoger():
    """An online logger class for calculating temporal segmentation metrics."""

    def __init__(self, overlap = [.1, .25, .5], ignore_index = -100, bg_class=["background"]):
    ...
    def update(self, labels, preds):
        # trim padding if avaliable
        if self.ignore_index:
            lim = np.where(labels == self.ignore_index)[0]
            if len(lim):
                lim = min(lim)
            else:
                lim = len(labels)
        ...
        self.edit += edit_score(preds, labels, bg_class=self.bg_class)
